...
public class MatchService {
	...
	public Report doSearch(Query query) {
		...
		try {
			...
			String uniref100 = searchParameters.getUniref100() ? "Y" : "N";
			...
			long totalFound = searchService.countByPeptide(
				query.getPeps(), taxonids, swissprot, isoform, uniref100, leqi);
			report.setNumberFound((int) totalFound);
			report.setQtime(0);
			report.setStatus("OK");
		} catch (IOException e) {
			report.setStatus("ERROR: " + e.getMessage());
		}
	...
	private ReportResults doESQuery(String queryPeptide, ReportSearchParameters searchParameters) {
		...
		try {
			...
			SearchResult searchResult = searchService.searchByPeptide(
				queryPeptide, taxonids, swissprot, isoform, uniref100, leqi,
				offset, size, "ac_asc");
			...
			for (Map<String, Object> hit : searchResult.hits()) {
				...
				String taxongroupName = (String) hit.get("taxongroupName");
				protein.setOrgTaxonGroupName(taxongroupName);

				Object taxongroupIDObj = hit.get("taxongroupID");
				Integer taxongroupID = Integer.valueOf(taxongroupIDObj.toString());
