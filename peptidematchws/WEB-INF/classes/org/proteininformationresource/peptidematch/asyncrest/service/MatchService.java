	...
	public void run() {
		...
		try {
			...
			for (int i = 0; i < queryPeptides.size(); i++) {
				...
				doSearch(peptideMatchQuery, queryPeptide, taxonIds, query.getlEqi());
				Date end = new Date();
...
//			}
	...
	private void doSearch(PeptideMatchPhraseQuery peptideMatchQuery, String queryPeptide, String queryTaxonId, String lEqi) {
		System.out.println("Query peptide: " + queryPeptide);
		SolrDocumentList docs = new SolrDocumentList();
		peptideMatchQuery.queryByPeptide(queryPeptide, 0, 1, "N", lEqi, "ac_asc");
		...
		if (numberFound > 0) {
			for (int i = 0; i < numberFound; i += numberPerPage) {
				peptideMatchQuery.queryByPeptideWithOrganism(queryPeptide, queryTaxonId, i, numberPerPage, "N", lEqi, "ac_asc");
