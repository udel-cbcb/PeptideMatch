...
public class PeptideMatchCMD {
	...
	public static void main(String[] args) {
		final int nGram = 3;
		String usage  = "java -jar PeptideMatchCMD_1.1.jar [options]\n";
		...
		options.addOption("l", "list", false, "The query peptide sequence file is a list of peptide sequences, one sequence per line.");
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				if (line.length() > 0) {
					index++;
					if (index == 1 && !line.startsWith(">")) {
						System.err.println("\"" + dataFilePath + "\" is not a valid Fasta file. Reason: first line must be started with \">\"\n");
						System.exit(1);
					}
					if (line.startsWith(">")) {
						if (!id.equals("") && !seq.equals("")) {
							addDoc(writer, id, seq, nGram, lEqi);
							id = "";
							seq = "";
						} else if (index > 1 && seq.equals("")) {
							System.err.println("\"" + dataFilePath + "\" is not a valid Fasta file. Reason: the sequence is missing for the id: " + id + "\n");
							System.exit(1);
						} else if (index > 1 && id.equals("")) {
							System.err.println("\"" + dataFilePath + "\" is not a valid Fasta file. Reason: the id is missing for the following sequences: \n" + seq + "\n");
							System.exit(1);
						}
						String gt = "^>";
						line = line.replaceAll(gt, "");
		...
		try {
			...
			for (int i = 0; i < queries.size(); i++) {
				...
				for (int j = 0; j <= queryStr.length() - nGram; j++) {
					if (lEqi) {
						nGramPhraseQueryOptimizer.add(new Term("lToiSeq", queryStr.substring(j, j + nGram)));
					} else {
						nGramPhraseQueryOptimizer.add(new Term("originalSeq", queryStr.substring(j, j + nGram)));
					}
				}
				...
				if (ilEquivalent) {
					...
					for (int j = i; j < i + queryPeptide.length(); j++) {
						...
						if (originalChar != replacedChar && originalChar != originalQueryPeptide.charAt(j - i)) {
							replacedPosList.add(Integer.valueOf(j + 1));
						}
					}
