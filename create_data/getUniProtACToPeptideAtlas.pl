...
if(@ARGV != 2) {
        print "perl getUniProtACToPeptideAtlas.pl sprot.dat trembl.dat\n";
        exit 1;
}
...
sub getOrgNameAndTaxonId {
	...
	while($line=<DATA>){
		...
		#NCBI_TaxID=115547;
		elsif($line =~ /^DR\s+PeptideAtlas\;/) {
                	$line =~ s/^DR\s+PeptideAtlas\;//;
                	$peptideAtlasId = (split(/\;/, $line))[0];
        	}

        	elsif ($line =~ /^\/\/$/) {
			$start = 0;
			if($peptideAtlasId) {
				print ">".$firstAC."\t".$peptideAtlasId."\n";
			}	
			$firstAC = "";
			$peptideAtlasId = "";
