...
if(request.getParameter("initialed")!=null &&request.getParameter("keyword")!=null &&request.getParameter("start")!=null &&request.getParameter("rows")!=null &&request.getParameter("organism_id")!=null &&request.getParameter("taxongroup_id")!=null &&request.getParameter("group_name")!=null &&request.getParameter("total_number")!=null){
...
String spectraSearch  ="<table border=0>\n";
	spectraSearch +="       <tr><td valign=top style=\"white-space: nowrap;\">Search query peptide in <a href=\"docs/userguide.htm#spectra\">proteomic DBs</a>: </td>\n";
       spectraSearch +="            <td valign=top><form action=\"peptidespectra.jsp\" name=\"spectrasearchform\" onSubmit=\"return spectraSearchInputCheck(this)\">\n";
...
<form id=inputform action=peptidewithorganisms.jsp method=get name="searchform" onSubmit="return InputCheck(this)">
Query peptide:&nbsp;&nbsp;<input style="width: 350px;" type=text name="keyword" id="keyword" onblur="reminder()" onfocus="if(!this._haschanged){this.value=''};this._haschanged=true;" value=<%if(request.getParameter("keyword")!=null)out.print(request.getParameter("keyword").toUpperCase()); %> > 
...
<%=version%><%
	...
	//out.print("ilEquivalent: "+ilEquivalent);
        if(ilEquivalent.equals("Y")) {
                //out.print(" | Leucine (L) and IsoLeucine (I) are equivalent");
                out.print(" | L and I are equivalent");
        }
	out.print("<br/>Query peptide: "+originalQuery.toUpperCase()+"<br/>");
	if(request.getParameter("group_name")!=null&&request.getParameter("group_name")!="") {
		out.print("Taxonomic Group:<a href='http://www.ncbi.nlm.nih.gov/Taxonomy/Browser/wwwtax.cgi?lvl=0&id="+request.getParameter("taxongroup_id")+"'/>"+request.getParameter("group_name")+"</a>");	
	}
%>
<!--
...
if(request.getParameter("group_name")!=null&&request.getParameter("group_name")!="")
	out.print(" & Group:<a href='http://www.ncbi.nlm.nih.gov/Taxonomy/Browser/wwwtax.cgi?lvl=0&id="+request.getParameter("taxongroup_id")+"'/>"+request.getParameter("group_name")+"</a>");	
else if(request.getParameter("taxongroup_id")!=null)
	out.print(" & Group:<a href='http://www.ncbi.nlm.nih.gov/Taxonomy/Browser/wwwtax.cgi?lvl=0&id="+request.getParameter("taxongroup_id")+"'/>"+request.getParameter("taxongroup_id")+"</a>");
else out.print(" & TaxonGroup:null");
			%>)
</b>
...
%>
<th width=11% id="ac_header" class="header" onclick="sortTable('false','<%=originalQuery%>','<%=organism_ID%>','','','<%=taxongroup_id%>','<%=group_name%>','0','20','<%=numberFound%>','<%=swissprotValue%>','<%=isoformValue%>','<%=uniref100Value%>','<%=lEqiValue%>',this.id)"><b>Protein AC</b></th>
<th width=9%  id="proteinID_header" class="header" onclick="sortTable('false','<%=originalQuery%>','<%=organism_ID%>','','','<%=taxongroup_id%>','<%=group_name%>','0','20','<%=numberFound%>','<%=swissprotValue%>','<%=isoformValue%>','<%=uniref100Value%>','<%=lEqiValue%>',this.id)"><b>Protein ID</b></th>
<th width=15% id ="proteinName_header" class="header" onclick="sortTable('false','<%=originalQuery%>','<%=organism_ID%>','','','<%=taxongroup_id%>','<%=group_name%>','0','20','<%=numberFound%>','<%=swissprotValue%>','<%=isoformValue%>','<%=uniref100Value%>','<%=lEqiValue%>',this.id)" ><b>Protein Name</b></th>
<th width=5%  id="length_header" class="header" onclick="sortTable('false','<%=originalQuery%>','<%=organism_ID%>','','','<%=taxongroup_id%>','<%=group_name%>','0','20','<%=numberFound%>','<%=swissprotValue%>','<%=isoformValue%>','<%=uniref100Value%>','<%=lEqiValue%>',this.id)" ><b>Length</b></th>
<th width=15% id ="organismName_header" class="header" onclick="sortTable('false','<%=originalQuery%>','<%=organism_ID%>','','','<%=taxongroup_id%>','<%=group_name%>','0','20','<%=numberFound%>','<%=swissprotValue%>','<%=isoformValue%>','<%=uniref100Value%>','<%=lEqiValue%>',this.id)"><b>Organism</b></th>
<th style="background-color:#C1D0DF"><b>Match Range</b></th>	
<th width=13% id="proteomic_header" class="header" onclick="sortTable('false','<%=originalQuery%>','<%=organism_ID%>','','','<%=taxongroup_id%>','<%=group_name%>','0','20','<%=numberFound%>','<%=swissprotValue%>','<%=isoformValue%>','<%=uniref100Value%>','<%=lEqiValue%>',this.id)"><b>Protein Links to Proteomic DBs</b></th>
<th width=10% id="iedb_header" class="header" onclick="sortTable('false','<%=originalQuery%>','<%=organism_ID%>','','','<%=taxongroup_id%>','<%=group_name%>','0','20','<%=numberFound%>','<%=swissprotValue%>','<%=isoformValue%>','<%=uniref100Value%>','<%=lEqiValue%>',this.id)" ><b>Immune Epitope DB</b></th>
<%
} 
 else {
%>
<th width=12% id="ac_header" class="header" onclick="sortTable('false','<%=originalQuery%>','<%=organism_ID%>','','','<%=taxongroup_id%>','<%=group_name%>','0','20','<%=numberFound%>','<%=swissprotValue%>','<%=isoformValue%>','<%=uniref100Value%>','<%=lEqiValue%>',this.id)"><b>UniRef100 Cluster ID</b></th>
<th width=12% id="ac_header" class="header" onclick="sortTable('false','<%=originalQuery%>','<%=organism_ID%>','','','<%=taxongroup_id%>','<%=group_name%>','0','20','<%=numberFound%>','<%=swissprotValue%>','<%=isoformValue%>','<%=uniref100Value%>','<%=lEqiValue%>',this.id)"><b>Representative Protein AC</b></th>
<th width=15% id ="proteinName_header" class="header" onclick="sortTable('false','<%=originalQuery%>','<%=organism_ID%>','','','<%=taxongroup_id%>','<%=group_name%>','0','20','<%=numberFound%>','<%=swissprotValue%>','<%=isoformValue%>','<%=uniref100Value%>','<%=lEqiValue%>',this.id)" ><b>Protein Name</b></th>
<th width=6%  id="length_header" class="header" onclick="sortTable('false','<%=originalQuery%>','<%=organism_ID%>','','','<%=taxongroup_id%>','<%=group_name%>','0','20','<%=numberFound%>','<%=swissprotValue%>','<%=isoformValue%>','<%=uniref100Value%>','<%=lEqiValue%>',this.id)" ><b>Length</b></th>
<th width=15% id ="organismName_header" class="header" onclick="sortTable('false','<%=originalQuery%>','<%=organism_ID%>','','','<%=taxongroup_id%>','<%=group_name%>','0','20','<%=numberFound%>','<%=swissprotValue%>','<%=isoformValue%>','<%=uniref100Value%>','<%=lEqiValue%>',this.id)"><b>Organism</b></th>
<th style="background-color:#C1D0DF"><b>Match Range</b></th>	
<th width=13% id="proteomic_header" class="header" onclick="sortTable('false','<%=originalQuery%>','<%=organism_ID%>','','','<%=taxongroup_id%>','<%=group_name%>','0','20','<%=numberFound%>','<%=swissprotValue%>','<%=isoformValue%>','<%=uniref100Value%>','<%=lEqiValue%>',this.id)"><b>Protein Links to Proteomic DBs</b></th>
<th width=11% id="iedb_header" class="header" onclick="sortTable('false','<%=originalQuery%>','<%=organism_ID%>','','','<%=taxongroup_id%>','<%=group_name%>','0','20','<%=numberFound%>','<%=swissprotValue%>','<%=isoformValue%>','<%=uniref100Value%>','<%=lEqiValue%>',this.id)" ><b>Immune Epitope DB</b></th>
<%
}
%>
<!--
...
<th width=9%  id="proteinID_header" class="header" onclick="sortTable('false','<%=originalQuery%>','<%=organism_ID%>','','','<%=taxongroup_id%>','<%=group_name%>','0','20','<%=numberFound%>','<%=swissprotValue%>','<%=isoformValue%>','<%=uniref100Value%>','<%=lEqiValue%>',this.id)"><b>Protein ID</b></th>
<th width=15% id ="proteinName_header" class="header" onclick="sortTable('false','<%=originalQuery%>','<%=organism_ID%>','','','<%=taxongroup_id%>','<%=group_name%>','0','20','<%=numberFound%>','<%=swissprotValue%>','<%=isoformValue%>','<%=uniref100Value%>','<%=lEqiValue%>',this.id)" ><b>Protein Name</b></th>
<th width=5%  id="length_header" class="header" onclick="sortTable('false','<%=originalQuery%>','<%=organism_ID%>','0','20','<%=numberFound%>','<%=swissprotValue%>','<%=isoformValue%>','<%=uniref100Value%>','<%=lEqiValue%>',this.id)" ><b>Length</b></th>
<th width=15% id ="organismName_header" class="header" onclick="sortTable('false','<%=originalQuery%>','<%=organism_ID%>','','','<%=taxongroup_id%>','<%=group_name%>','0','20','<%=numberFound%>','<%=swissprotValue%>','<%=isoformValue%>','<%=uniref100Value%>','<%=lEqiValue%>',this.id)"><b>Organism</b></th>
<th style="background-color:#C1D0DF"><b>Match Range</b></th>	
<th width=10% id="proteomic_header" class="header" onclick="sortTable('false','<%=originalQuery%>','<%=organism_ID%>','','','<%=taxongroup_id%>','<%=group_name%>','0','20','<%=numberFound%>','<%=swissprotValue%>','<%=isoformValue%>','<%=uniref100Value%>','<%=lEqiValue%>',this.id)"><b>Proteomic Databases</b></th>
<th width=10% id="iedb_header" class="header" onclick="sortTable('false','<%=originalQuery%>','<%=organism_ID%>','','','<%=taxongroup_id%>','<%=group_name%>','0','20','<%=numberFound%>','<%=swissprotValue%>','<%=isoformValue%>','<%=uniref100Value%>','<%=lEqiValue%>',this.id)" ><b>IEDB</b></th>
-->
</tr>
</thead>
...
<input style="display:none" id="constraint" value="withtaxongroup"></input>
...
<%	out.println("<script language='javascript'>window.onload=paging(\"false\",\""+originalQuery+"\",\""+organism_ID+"\",\"\",\"\",\""+taxongroup_id+"\",\""+group_name+"\",\"0\",\"20\",\""+numberFound+"\",\""+swissprotValue+"\",\""+isoformValue+"\",\""+uniref100Value+"\",\""+lEqiValue+"\",\""+sortBy+"\",\""+trOnly+"\",\""+isoOnly+"\");</script>");

//out.println("<script language='javascript'>window.onload=getPageContentWithTaxOnId(\"false\",\""+originalQuery+"\",\""+organism_ID+"\",\""+taxon_id+"\",\""+taxon_name+"\",\"0\",\"20\",\""+numberFound+"\",\""+swissprotValue+"\",\""+isoformValue+"\",\""+uniref100Value+"\",\""+lEqiValue+"\",\""+sortBy+"\",""+trOnly+"\",\""+isoOnly+"\");</script>");
