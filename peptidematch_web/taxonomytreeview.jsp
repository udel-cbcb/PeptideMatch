...
<%
	...
	String treeView = (String)session.getAttribute("TaxonomyTreeView");
	 ...
	 if(peptide.length() >= 3) {
		if(treeView == null) {
			...
			treeView = ((TreeView)buildTaxonomyTree(organismHitMap, organismHitNameMap, organismHitLineageMap, out)).getTreeView();
                        ...
                        session.setAttribute("TaxonomyTreeView", treeView);   
                }
                else {
                        treeView = (String)session.getAttribute("TaxonomyTreeView");
                }
	}
	else {
...
<table style="font-size: 11px; margin-left: 50px; width:100%"> 
	<tr>
  	...
  	<div style="width:95%;">
		...
		<!--
      		<label><a href="taxonomytableview.jsp?peptide=<%=peptide%>&organism_id=<%=organism_id%>&numberfound=<%=numberFound%>&view=table"><span>Lineage Table</span></a> | <span style="color: blue;">Taxonomy Tree</span></label>
		-->
      		<label><span style="color: blue;">Taxonomy Tree</span></label>
    		</div>
  	 </div>
	<br>
		<div id="sidetree">
...
<%!
	private TreeView buildTaxonomyTree(HashMap organismHitMap, HashMap organismHitNameMap, HashMap organismHitLineageMap, JspWriter out) {
		String lst = "";
