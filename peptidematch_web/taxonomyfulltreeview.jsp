	...
	if(peptide != null && organismId != null && numberFound != null) {
	 	...
	 	if(peptide.length() >= 3) {
			for(int i=0; i < taxonRec.length; i++) {
				...
				if(peptide.equals("*:*")){
					...
					if(!organismId.toLowerCase().equals("all")){
        					peptideQuery.queryByPeptideWithFullLineageOrganismAndTaxonId(peptide, organismId,taxonId, start, rowsPerPage, op, uniref100Only, ilEquivalent, sortBy);
					}
					else{
        					peptideQuery.queryByPeptideWithFullLineageTaxonId(peptide, taxonId, start, rowsPerPage, op, uniref100Only, ilEquivalent, sortBy);
					}
				}
			...
			if(totalMatch > 0) {
				...
				TreeMap childrenNodes = new TreeMap();
					...
					if(node.getName().equals("root")) {
						taxonomyTree += "<li id='"+node.getId()+"'><span style='font-weight: bold; color: navy;'>"+node.getName()+"</span>";
						    ...
						    childrenNodes.put(node.getName().toUpperCase(), "<li class='jstree-closed' id='"+node.getId()+"'>"+taxonNameUrl+seqCountUrl);
					}
                                }
				...
				session.setAttribute("currentQuery", peptide);
				session.setAttribute("currentOrganism", organismId);
                        }	
		}
		else {
	...
	<td>
  	<div style="width:95%;">
    		<div class="grid-header" style="width:100%; font-size: 15px;" >
      		<label><span style="font-weight: bold; font-size: 14px;">Taxonomy Tree View</span> | <span style="font-weight: bold; font-size: 14px;"><a href="peptidewithorganisms.jsp?keyword=<%=peptide%>&start=0&initialed=false&organism_id=<%=organismId%>&rows=20">Back to Table View</a></span></label>
    		</div>
  	 </div>
	<br>
...
<td align="left" valign="bottom" nowrap="">
            ...
            <tr> 
              <td colspan="2" align="center" nowrap="" class="footer3"><span class="nrm10"><font color="#999999"> &copy; 2015</font>
