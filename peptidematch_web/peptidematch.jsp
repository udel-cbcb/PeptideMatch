...
<%@ page import="query.NGramQuery"%>
<%@ page import="query.HighLight"%>
<html>
<head>
...
body.main {
	background-image:
		url('http://pir.georgetown.edu/pirwww/images/temp/bg02.gif');
...
<body class="main">
	...
	<table cellspacing="0" cellpadding="0" width="100%" border="0"
		...
		<tbody>
			<tr>
				<td><img alt="" src="./imagefiles/PIRBanner.png" border="0"
					usemap="#PIRBanner_Map"></td>
				<td width="99%" nowrap="" height="1"
					background="./imagefiles/blueSpacer.png"><img alt=""
					src="./imagefiles/spacer.gif" width="1" height="1" border="0">
				</td>
				<td align="right" valign="top" height="80" width="400"
					background="./imagefiles/gradientHome6.png">
					<table border="0" cellpadding="0" cellspacing="0" width="400"
						height="80">
						<form action="http://pir.georgetown.edu/cgi-bin/textsearch.pl"
							id="textSearch" method="get" name="textSearchForm"
							style="MARGIN: 0px"></form>
						<tbody>
							<tr>
								<td width="400" colspan="10"><img alt=""
									src="./imagefiles/spacer.gif" width="400" height="30"></td>
							</tr>
							<tr>
								<td><img alt="" src="./imagefiles/spacer.gif" width="60"
									height="1"></td>
								<td><input type="radio" name="sitesearch" checked=""></td>
								<td align="right" nowrap="" class="searchLable">Protein
									Search</td>
								<td><input type="radio" name="sitesearch"
									value="sitesearch"></td>
								<td align="right" nowrap="" class="searchLable">Site Search</td>
								<td><img alt="" src="./imagefiles/spacer.gif" width="8"
									height="1"></td>
								<td align="right"><input alt="submit" type="image"
									src="./imagefiles/but06.png" border="0" width="18" height="16"
									name="submit"></td>
								<td><img alt="" src="./imagefiles/spacer.gif" width="2"
									height="1"></td>
								<td align="right"><input alt="query" name="query0"
									type="text" class="searchBannerBox"
									style="background: #CED9E7;" value=""></td>
								<td><img alt="" src="./imagefiles/spacer.gif" width="6"
									height="1"></td>
							</tr>
	...
	//if open the page directly,don't show the result table
		if (request.getParameter("initialed") != null) {
			...
			NGramQuery peptideQuery = new NGramQuery();
			SolrDocumentList docs = new SolrDocumentList();
			//define the 
			int numberFound = 0;
			HighLight highLight = new HighLight();
			String originalQuery = request.getParameter("keyword").trim().toUpperCase();
			int start = Integer.parseInt(request.getParameter("start"));
			int rowsPerPage = Integer.parseInt(request.getParameter("rows"));
			String searchType=request.getParameter("searchtype");
			//check the search type
			if (searchType.equals("peptide")) {
				//check the query length when the search type is peptide
				//if the length is greater than 5, we need to query the long peptide search, or get from the session
				if(originalQuery.length()==3) {
						docs = peptideQuery.queryByShortPeptide(
								originalQuery, rowsPerPage, start);
						numberFound = peptideQuery.getResult();
					}
				else if (originalQuery.length() > 3) {
					//if the session is timeout or the user input a new keyword, we perform a new query
				if(request.getParameter("initialed").equals("false")){
					peptideQuery.queryByLongPeptide(originalQuery,
							rowsPerPage, start);
					docs = peptideQuery.getInitialDoc();
					//store the id sets of protein sequences which are matched the keyword
					session.setAttribute("idset",peptideQuery.getIDSets());
					session.setAttribute("currentquery",originalQuery);
					//return how many protein sequence 
					numberFound = peptideQuery.getResult();
				}else{
					if(session.getAttribute("idset") != null&& ((String)session.getAttribute("currentquery")).equals(originalQuery) ){
						
						docs = new SolrDocumentList();
					numberFound = Integer.parseInt(request
							.getParameter("numberfound"));
					ArrayList<String> IDSets = new ArrayList();
					ArrayList<String> fullIDSets = (ArrayList<String>)session.getAttribute("idset");
					//if the the start point added by the rows Perpage is not greater than total, then just from start to start+rows PerPage
					if((start+rowsPerPage)<numberFound){
					for (int i = start; i < (start + rowsPerPage); i++) {	
					//	out.println(fullIDSets.get(i));
						IDSets.add(fullIDSets.get(i));
					}
					}
					else{
						for (int i = start; i < numberFound; i++) {
							IDSets.add(fullIDSets.get(i));
					}
					}
				//	out.println(IDSets);
					docs = peptideQuery.queryByIDSets(IDSets,rowsPerPage);
				//	out.println(docs);
					}else{
					...
					out.println("The keyword must be at least 3 characters");
				}
				}
			else if (searchType.equals("id")) {
				docs = peptideQuery.queryByID(originalQuery);
				numberFound = peptideQuery.getResult();
			}
			session.setAttribute("currentdocs",docs);
	%>

	<form class=inputform action=peptidematch.jsp method=get
		name="searchform" onSubmit="return InputCheck(this)">
		Query peptide:&nbsp;&nbsp;<input type=text name="keyword"
			class="keyword" value=<%=originalQuery %>
			onfocus="if(!this._haschanged){this.value=''};this._haschanged=true;">
		<input type="hidden" name="start" value=0> <input
			type="hidden" name="initialed" value="false"> <input
			type="hidden" name="searchtype" value="peptide"> <input
			type="hidden" name="rows" value="20"> <input type=submit
	...
	<table class="headline" width="100%" border="0" cellpadding="0"
		cellpadding="0">
		<tbody>
			<tr>
				<td></td>
			...
			<tr>
				<td id=headresult><%=numberFound%> Proteins
					Matched&nbsp;|&nbsp;Page <%=start/rowsPerPage+1 %> of <%=(numberFound-1)/rowsPerPage+1 %>&nbsp;(<%=rowsPerPage%>
					Rows/Page)</td>
				<td align=right>
					<table border="0" cellpadding="0" cellspacing="0">
						<tbody>
							<tr>
								<td align="right"><img alt="save"
									src="./imagefiles/save_as.png"></td>
								<td class="nrm02" width="4" align="center">&nbsp;</td>
								<td align="center"><a
									href=<% out.print("./"+response.encodeUrl("SaveData")+"?filetype=tab&query="+originalQuery+"&start="+start); %>><input
										type="image" src="./imagefiles/save_fxn_tbl.jpg"></td>
								<a></td>
									<td class="nrm02" width="4" align="center">|</td>
									<td align="center"><a
										href=<% out.print("./"+response.encodeUrl("SaveData")+"?filetype=fasta&query="+originalQuery+"&start="+start); %>><input
											alt="save FASTA" type="image" name="save_fasta"
											src="./imagefiles/save_fxn_fasta.jpg"></a></td>
									<td><img alt="" src="./imagefiles/transparent_dot.png"
										border="0" height="2" width="50"></td>
							</tr>
		...
		Iterator<SolrDocument> docItr = docs.iterator();
			...
			<tr>
				...
				<td class="tablecontent">
					<% out.println(id+"/"+proteinID); %><br> <a
					href="http://pir.georgetown.edu/cgi-bin/ipcEntry?id=<%=id%>"><img
						src="./imagefiles/ipc_icon.png" border="0"></a> <a
					href="http://www.uniprot.org/uniprot/<%=id%>"><img
						src="./imagefiles/tr_icon.png" border="0"></a>
				</td>
				<%} else { %>
				<td class="tablecontent">
					<%out.println(id+"/"+proteinID);%>
				</td>
				<%}	%>
				<td class="tablecontent"><%=ProteinName%><br> <a
					href='http://pir.georgetown.edu/cgi-bin/biothesaurus.pl?id=<%=id%>'><img
						src="./imagefiles/biot_icon2.png" border="0"> </a></td>
				<td class="tablecontent"><%=sequence.length()%></td>
				<td class="tablecontent"><a
					href='http://www.ncbi.nlm.nih.gov/Taxonomy/Browser/wwwtax.cgi?lvl=0&id=<%=organismID%>'><%=organism%></a></td>
				<td class="tablecontent">
					<%
				...
				<td class="tablecontent" id="sequencefragment">
					<%
				if (!request.getParameter("searchtype").equals("id")) {
					for(int i=0;i<=sequence.length()-originalQuery.length();i++){
						if(sequence.substring(i, i+originalQuery.length()).equals(originalQuery)){
						//	out.println("<p id=sequencefragment>");
							out.print(i);
							out.print("-");
						...
						if(request.getParameter("hl")!=null){
							out.println(highLight.highLight(sequence, (String)request.getParameter("hl")));
						}else
	...
	//if the number sequences returned is greater than rows per page , need to paging
		if (numberFound > rowsPerPage) {
			if( start/ rowsPerPage >6){
				out.print("<a href='./"
						+response.encodeURL("peptidematch.jsp")
											...
											<tr>
												...
												<td nowrap="" align="center" class="footer"><a
													href="http://bioinformatics.udel.edu/"><font
														color="#999999">University of Delaware</font></a> <br>
												<font color="#999999">15 Innovation Way, Suite 205 <br>Newark,
														DE 19711, USA
												</font></td>
												<td><img src="./imagefiles/spacer.gif" alt=""
												...
												<td nowrap="" align="center" class="footer"><a
													href="http://gumc.georgetown.edu/"><font
														color="#999999">Georgetown University Medical
															Center</font></a> <br>
												<font color="#999999">3300 Whitehaven Street, NW,
														Suite 1200 <br>Washington, DC 20007, USA
												</font></td>
											</tr>
										</tbody>
									</table></td>
