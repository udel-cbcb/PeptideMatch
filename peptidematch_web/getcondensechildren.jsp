...
<%
	...
	Map<String, Long> sortedHitOrganismsCount = new LinkedHashMap();
        ...
        if(!organismId.toLowerCase().equals("all")) {
        	if(session.getAttribute("organismsCount-"+peptide+"-"+organismId+"-"+swissprot+"-"+isoform+"-"+uniref100Only+"-"+ilEquivalent+"-"+trOnly+"-"+isoOnly) == null) {
           		sortedHitOrganismsCount = peptideQuery.queryByPeptideWithMultiOrganismWithGroup(peptide, organismId, swissprot, isoform, uniref100Only, ilEquivalent, sortBy, trOnly, isoOnly);
                        session.setAttribute("organismsCount-"+peptide+"-"+organismId+"-"+swissprot+"-"+isoform+"-"+uniref100Only+"-"+ilEquivalent+"-"+trOnly+"-"+isoOnly, sortedHitOrganismsCount);
                }
                else {
                       sortedHitOrganismsCount = (Map<String, Long>)session.getAttribute("organismsCount-"+peptide+"-"+organismId+"-"+swissprot+"-"+isoform+"-"+uniref100Only+"-"+ilEquivalent+"-"+trOnly+"-"+isoOnly);
                }
		candidateChildren = (ArrayList)session.getAttribute("candidateChildren-"+peptide+"-"+organismId+"-"+swissprot+"-"+isoform+"-"+uniref100Only+"-"+ilEquivalent+"-"+trOnly+"-"+isoOnly); 
		if(candidateChildren == null) {
			candidateChildren =  getCandidateChildren(request, sortedHitOrganismsCount);
			session.setAttribute("candidateChildren-"+peptide+"-"+organismId+"-"+swissprot+"-"+isoform+"-"+uniref100Only+"-"+ilEquivalent+"-"+trOnly+"-"+isoOnly, candidateChildren);
		}
        }
        else {
        	if(session.getAttribute("organismsCount-"+peptide+"-all"+"-"+swissprot+"-"+isoform+"-"+uniref100Only+"-"+ilEquivalent+"-"+trOnly+"-"+isoOnly) == null) {
                	sortedHitOrganismsCount = peptideQuery.queryByPeptideWithGroup(peptide, swissprot, isoform, uniref100Only, ilEquivalent, sortBy, trOnly, isoOnly);
		...
		candidateChildren = (ArrayList)session.getAttribute("candidateChildren-"+peptide+"-all"+"-"+swissprot+"-"+isoform+"-"+uniref100Only+"-"+ilEquivalent+"-"+trOnly+"-"+isoOnly); 
		if(candidateChildren == null) {
			candidateChildren =  getCandidateChildren(request, sortedHitOrganismsCount);
			session.setAttribute("candidateChildren-"+peptide+"-all"+"-"+swissprot+"-"+isoform+"-"+uniref100Only+"-"+ilEquivalent+"-"+trOnly+"-"+isoOnly, candidateChildren);
		}
       }
				  		...
				  		if(taxonId.equals(openedNodeIdTaxon)) {
							...
							for(int i=0; i < taxonRec.length; i++) {
								...
								if(peptide.equals("*:*")){
									...
									if(!organismId.toLowerCase().equals("all")){
        									peptideQuery.queryByPeptideWithShortLineageOrganismAndTaxonId(peptide, organismId,taxonId, start, rowsPerPage, op, swissprot, isoform, uniref100Only, ilEquivalent, sortBy, trOnly, isoOnly);
									}
									else{
        									peptideQuery.queryByPeptideWithShortLineageTaxonId(peptide, taxonId, start, rowsPerPage, op, swissprot, isoform, uniref100Only, ilEquivalent, sortBy, trOnly, isoOnly);
									}
								}
					...
					HashMap childrenNodes = new HashMap();
						...
						if(node.getName().equals("root")) {
							...
							//String nodeStr = node.getName() + "	
						    	...
						    	//String seqCountUrl = " (<a href='peptidewithlineagetaxonid.jsp?initialed=false&start=0&rows=20&keyword="+peptide+"&organism_id="+organismId+"&total_number="+numberFound+"&taxon_name="+node.getName().replaceAll("'", "&#39;")+"&taxon_id="+node.getTaxonomyID()+"&swissprot="+swissprotValue+"&isoform="+isoformValue+"&uniref100="+uniref100Value+"&lEqi="+lEqiValue+"&sortBy="+sortBy+"&trOnly="+trOnly+"&isoOnly="+isoOnly+"'>"+node.getSeqCount()+"</a>)";
