...
function organism_list(str){
	...
	var selected ="";
	//alert(len);
	if(len==0) newlist.push(str);
	for(var i=0;i<len;i++){
	//	alert("test");
		if(organismlist[i]==str){
			exist=true;
			continue;
		} 
		else {
			newlist.push(organismlist[i]);
			selected += organismlist[i];
		}
	}
	if(!exist&len>=1) newlist.push(str);
	...
	document.getElementById("show_selected_count").innerHTML='(<b>'+selected+' selected</b>)';  
	}
...
//show the taxon multiple selection	
	...
	{	
	var objs=document.getElementsByClassName("abanner");
...
//change the size and display of the taxon multiple selection	 	
	...
	document.getElementById("th_formtitle").style.width='85%'
...
// close the taxon multiple selection	
	...
	document.getElementById("th_formtitle").style.width='75%'
	document.getElementById("th_link").style.width='25%'
