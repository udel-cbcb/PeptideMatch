 ...
 * 
 */
public class TaxonomyTreeNode implements Comparable<TaxonomyTreeNode> {
	public TaxonomyTreeNode(String name, int taxonomyID, String type) {
		super();
		this.name = name;
	...
	public TaxonomyTreeNode(String name, int taxonomyID, String type, int seqCount) {
		super();
		this.name = name;
		this.taxonomyID = taxonomyID;
	...
	private TaxonomyTreeNode parent;
	private TaxonomyTreeNode[] children = new TaxonomyTreeNode[0];
	private String name;
	private String type;
	...
	public TaxonomyTreeNode getParent() {
		return parent;
	}

	public void setParent(TaxonomyTreeNode parent) {
		this.parent = parent;
	}

	public TaxonomyTreeNode[] getChildren() {
		return children;
	}

	public void setChildren(TaxonomyTreeNode[] children) {
	...
	public void add(TaxonomyTreeNode child, int index) {
		// Add the child to the list of children.
		if (index < 0 || index == children.length) // then append
		{
			TaxonomyTreeNode[] newChildren = new TaxonomyTreeNode[children.length + 1];
	 ...
	 */
	public void add(TaxonomyTreeNode child) {
		add(child, -1);
	...
	public TaxonomyTreeNode remove(int index) {
		...
		TaxonomyTreeNode node = children[index];
		node.parent = null;
	 ...
	 *         not <code>null</code>.
	 */
	public TaxonomyTreeNode[] children() {
		return children;
	}
	...
	public boolean hasChildren(TaxonomyTreeNode child) {
	 ...
	 */
	private int recurseDepth(TaxonomyTreeNode node, int depth) {
...
/*
	private String printTree() {
		...
		for (int i = 0; i < children.length; i++) {
			TaxonomyTreeNode child = children[i];
			s += "\n" + child.printTree();
		}
		for (int i = 0; i < children.length; i++) {
			TaxonomyTreeNode child = children[i];
	...
	*/
	...
	public String printTree(int increment) {
	 ...
	 public String printTree() {
		...
		if(!this.getType().equals("") && !this.getType().equals("no rank")) {
			lst +=" [<font color=blue>" + this.getType()+"</font>]";
		}
			lst += " (" + this.getSeqCount()+")";
		
		if(this.children.length > 0) {
			lst +="<ul>\n";
			Arrays.sort(children, TaxonomyTreeNode.TaxonomyTreeNodeNameComparator);
			//for(int i=0; i < this.children.length; i++) {
			for(TaxonomyTreeNode temp: children) {
	...
	public String printLineage() {
	 ...
	 Queue<TaxonomyTreeNode> one = new LinkedList<TaxonomyTreeNode>();
	Queue<TaxonomyTreeNode> two = new LinkedList<TaxonomyTreeNode>();

	//static void printLevelList(TaxonomyTreeNode root) {
 void printLevelList() {
		...
		while(true) {
			while(!one.isEmpty()) {
				TaxonomyTreeNode node = one.remove();
 ...
 void printLineage1() {
	...
	private  void addToQueue(TaxonomyTreeNode[] children) {
		if (children.length == 0) {
			return;
		}
		for (int i = 0; i < children.length; i++) {
			two.add((TaxonomyTreeNode) children[i]);
		}
	}
	...
	public int compareTo(TaxonomyTreeNode compareTaxonomyTreeNode) {
			return this.name.compareTo(compareTaxonomyTreeNode.getName());
	}
	
	public static Comparator<TaxonomyTreeNode> TaxonomyTreeNodeNameComparator = new Comparator<TaxonomyTreeNode>() {
		public int compare(TaxonomyTreeNode node1, TaxonomyTreeNode node2) {
