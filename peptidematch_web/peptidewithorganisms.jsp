...
if(request.getParameter("initialed")!=null &&request.getParameter("keyword")!=null &&request.getParameter("start")!=null &&request.getParameter("rows")!=null &&request.getParameter("organism_id")!=null){
	...
	if(session.getAttribute("orgIdToNameMap") == null) {
               	...
               	session.setAttribute("orgIdToNameMap", orgIdToNameMap);
	}
	else {
		orgIdToNameMap = (HashMap<String, String>)session.getAttribute("orgIdToNameMap");
	}
...
<%=version%><%
	...
	out.print("<br/>Query peptide: <b>"+originalQuery.toUpperCase()+"</b><br/>");
	String[] organisms=request.getParameter("organism_id").toLowerCase().split(";");
	if(!request.getParameter("organism_id").equals("all") && organisms.length >= 0) {	
		if(organisms.length == 1) {
			out.print("Query organism: ");
		}
		else {
			out.print("Query organism(s): ");
		}
		for(int i=0;i<organisms.length;i++){
			out.print("<a href='http://www.ncbi.nlm.nih.gov/Taxonomy/Browser/wwwtax.cgi?lvl=0&id="+organisms[i]+"'>"+orgIdToNameMap.get(organisms[i])+"</a>");
...
<!--
<td width=40% align=right style="white-space: nowrap;" valign=top>
<%=spectraSearch%>
</td>
-->
<!--
...
for(int i=0;i<organisms.length;i++){
	if(hitOrgs.contains(organisms[i])) {
		out.print("<a href='http://www.ncbi.nlm.nih.gov/Taxonomy/Browser/wwwtax.cgi?lvl=0&id="+organisms[i]+"'>"+orgIdToNameMap.get(organisms[i])+"</a>");
		if(i!=organisms.length-1) {
			out.print("; ");
		}
...
<tr>
	...
	<table  class="" width="100%" border="0" cellpadding="0" cellpadding="0">
		<tbody> <tr>
			...
			<td id="browse_by" style="display:none" align=right>
                         Browse by <b> <span id="browse_link"><a href="#">Organisms</a></b></span> or <b>
			<% out.print("<b><a href=taxongroup.jsp?peptide="+originalQuery+"&organism_id="+organism_ID+"&numberfound="+numberFound+"&swissprot="+swissprotValue+"&isoform="+isoformValue+"&uniref100="+uniref100Value+"&lEqi="+lEqiValue+"&sortBy="+sortBy+"&trOnly="+trOnly+"&isoOnly="+isoOnly+">Taxonomic Group</a></b> or</b> "); %>
			<% out.print("<b><a href=taxonomycondensetreeview.jsp?peptide="+originalQuery+"&organism_id="+organism_ID+"&numberfound="+numberFound+"&swissprot="+swissprotValue+"&isoform="+isoformValue+"&uniref100="+uniref100Value+"&lEqi="+lEqiValue+"&sortBy="+sortBy+"&trOnly="+trOnly+"&isoOnly="+isoOnly+">Taxonomy Tree</a></b>"); %>
                        </td>
			</tr>
			</table>
			</td>
...
<form class="mainform" id="mainform"  name="mainform" method="post"  action="http://pir.georgetown.edu/cgi-bin/peptidematch.pl"> 
...
%>
<th width=11% id="ac_header" class="header" onclick="sortTable('false','<%=originalQuery%>','<%=organism_ID%>','','','','','0','20','<%=numberFound%>','<%=swissprotValue%>','<%=isoformValue%>','<%=uniref100Value%>','<%=lEqiValue%>','<%=trOnly%>', '<%=isoOnly%>',this.id)"><b>Protein AC&nbsp;&nbsp;</b></th>
<th width=9%  id="proteinID_header" class="header" onclick="sortTable('false','<%=originalQuery%>','<%=organism_ID%>','','','','','0','20','<%=numberFound%>','<%=swissprotValue%>','<%=isoformValue%>','<%=uniref100Value%>','<%=lEqiValue%>','<%=trOnly%>','<%=isoOnly%>', this.id)"><b>Protein ID&nbsp;&nbsp;</b></th>
<th width=15% id ="proteinName_header" class="header" onclick="sortTable('false','<%=originalQuery%>','<%=organism_ID%>','','','','','0','20','<%=numberFound%>','<%=swissprotValue%>','<%=isoformValue%>','<%=uniref100Value%>','<%=lEqiValue%>','<%=trOnly%>','<%=isoOnly%>',this.id)" ><b>Protein Name&nbsp;&nbsp;</b></th>
<th width=5%  id="length_header" class="header" onclick="sortTable('false','<%=originalQuery%>','<%=organism_ID%>','','','','','0','20','<%=numberFound%>','<%=swissprotValue%>','<%=isoformValue%>','<%=uniref100Value%>','<%=lEqiValue%>','<%=trOnly%>','<%=isoOnly%>',this.id)" ><b>Length&nbsp;&nbsp;</b></th>
<th width=15% id ="organismName_header" class="header" onclick="sortTable('false','<%=originalQuery%>','<%=organism_ID%>','','','','','0','20','<%=numberFound%>','<%=swissprotValue%>','<%=isoformValue%>','<%=uniref100Value%>','<%=lEqiValue%>','<%=trOnly%>','<%=isoOnly%>',this.id)"><b>Organism&nbsp;&nbsp;</b></th>
<th style="background-color:#C1D0DF"><b>Match Range</b></th>	
<th width=13% id="proteomic_header" class="header" onclick="sortTable('false','<%=originalQuery%>','<%=organism_ID%>','','','','','0','20','<%=numberFound%>','<%=swissprotValue%>','<%=isoformValue%>','<%=uniref100Value%>','<%=lEqiValue%>','<%=trOnly%>','<%=isoOnly%>',this.id)"><b>Protein Links to Proteomic DBs&nbsp;&nbsp;</b></th>
<th width=10% id="iedb_header" class="header" onclick="sortTable('false','<%=originalQuery%>','<%=organism_ID%>','','','','','0','20','<%=numberFound%>','<%=swissprotValue%>','<%=isoformValue%>','<%=uniref100Value%>','<%=lEqiValue%>','<%=trOnly%>','<%=isoOnly%>',this.id)" ><b>Immune Epitope DB&nbsp;&nbsp;</b></th>
<%
} 
 else {
%>
<th width=13% id="ac_header" class="header" onclick="sortTable('false','<%=originalQuery%>','<%=organism_ID%>','','','','','0','20','<%=numberFound%>','<%=swissprotValue%>','<%=isoformValue%>','<%=uniref100Value%>','<%=lEqiValue%>','<%=trOnly%>','<%=isoOnly%>',this.id)"><b>UniRef100 Cluster ID&nbsp;&nbsp;</b></th>
<th width=11% id="ac_header" class="header" onclick="sortTable('false','<%=originalQuery%>','<%=organism_ID%>','','','','','0','20','<%=numberFound%>','<%=swissprotValue%>','<%=isoformValue%>','<%=uniref100Value%>','<%=lEqiValue%>','<%=trOnly%>','<%=isoOnly%>',this.id)"><b>Representative Protein AC&nbsp;&nbsp;</b></th>
<th width=15% id ="proteinName_header" class="header" onclick="sortTable('false','<%=originalQuery%>','<%=organism_ID%>','','','','','0','20','<%=numberFound%>','<%=swissprotValue%>','<%=isoformValue%>','<%=uniref100Value%>','<%=lEqiValue%>','<%=trOnly%>','<%=isoOnly%>',this.id)" ><b>Protein Name&nbsp;&nbsp;</b></th>
<th width=6%  id="length_header" class="header" onclick="sortTable('false','<%=originalQuery%>','<%=organism_ID%>','','','','','0','20','<%=numberFound%>','<%=swissprotValue%>','<%=isoformValue%>','<%=uniref100Value%>','<%=lEqiValue%>','<%=trOnly%>','<%=isoOnly%>',this.id)" ><b>Length&nbsp;&nbsp;</b></th>
<th width=15% id ="organismName_header" class="header" onclick="sortTable('false','<%=originalQuery%>','<%=organism_ID%>','','','','','0','20','<%=numberFound%>','<%=swissprotValue%>','<%=isoformValue%>','<%=uniref100Value%>','<%=lEqiValue%>','<%=trOnly%>','<%=isoOnly%>',this.id)"><b>Organism&nbsp;&nbsp;</b></th>
<th style="background-color:#C1D0DF"><b>Match Range</b></th>	
<th width=13% id="proteomic_header" class="header" onclick="sortTable('false','<%=originalQuery%>','<%=organism_ID%>','','','','','0','20','<%=numberFound%>','<%=swissprotValue%>','<%=isoformValue%>','<%=uniref100Value%>','<%=lEqiValue%>','<%=trOnly%>','<%=isoOnly%>',this.id)"><b>Protein Links to Proteomic DBs&nbsp;&nbsp;</b></th>
<th width=11% id="iedb_header" class="header" onclick="sortTable('false','<%=originalQuery%>','<%=organism_ID%>','','','','','0','20','<%=numberFound%>','<%=swissprotValue%>','<%=isoformValue%>','<%=uniref100Value%>','<%=lEqiValue%>','<%=trOnly%>','<%=isoOnly%>',this.id)" ><b>Immune Epitope DB&nbsp;&nbsp;</b></th>
<%
}
%>
</tr>
...
<input style="display:none" id="constraint" value="withorganisms"></input>
...
<%	
//out.print("<script language='javascript'>showHitSummary(\"false\",\""+originalQuery+"\",\""+organism_ID+"\",\"0\",\"20\",\""+numberFound+"\",\""+swissprotValue+"\",\""+isoformValue+"\",\""+uniref100Value+"\",\""+lEqiValue+"\",\""+sortBy+"\");</script>");
out.print("<script language='javascript'>showHitSummary('false','"+originalQuery+"','"+organism_ID+"','0','20','"+numberFound+"','"+swissprotValue+"','"+isoformValue+"','"+uniref100Value+"','"+lEqiValue+"','"+sortBy+"','"+trOnly+"','"+isoOnly+"');</script>");
//out.print("<script language='javascript'>paging('false','"+originalQuery+"','"+organism_ID+"','0','20','"+numberFound+"','"+swissprotValue+"','"+isoformValue+"','"+uniref100Value+"','"+lEqiValue+"','"+sortBy+"','"+trOnly+"','"+isoOnly+"');</script>");
out.println("<script language='javascript'>paging('false','"+originalQuery+"','"+organism_ID+"','','','','','0','20','"+numberFound+"','"+swissprotValue+"','"+isoformValue+"','"+uniref100Value+"','"+lEqiValue+"','"+sortBy+"','"+trOnly+"','"+isoOnly+"');</script>");
}
    else {
	%>
...
<tr><td><span id="div-paging"></span></td></tr>
<tr><td>
<br/>
